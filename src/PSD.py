...
def assignPhaseValues(seeds, originalCoord, directions, normV, point1, point2, nodes):
    ...
    while phaseCount < len(originalCoord):
        ...
        gammaPhi[0, (phaseCount * seeds): (phaseCount + 1) * seeds] = makeCircle3D(originalCoord[phaseCount], radius,
                                                                                   seeds, normV[phaseCount],
                                                                                   point1[phaseCount],
                                                                                   point2[phaseCount], nodes)
...
def eikonal_solver(nodes, triangles, knownNodes, phi0):
    ...
    while (updateChange > tol) and (iterationNum < maxIterations):
        ...
        # matrix A
        A = Diff \
            + 3. / 4 * Tt2v.dot(dia_matrix((np.concatenate(cv, axis=0), [0]), shape=(nt, nt)).dot(
            np.imag(dia_matrix((np.concatenate(Tv2t.dot(phi), axis=0), [0]), shape=(nt, nt)).dot(B)))) \
            + 1. / 4 * np.imag(dia_matrix((np.concatenate(phi, axis=0), [0]), shape=(nv, nv)).dot(Tt2v).dot(
            dia_matrix((np.concatenate(cv, axis=0), [0]), shape=(nt, nt)).dot(B)))
...
def PSD(args, job, cmd, meshname, xyz, triangles, centre):
    ...
    initialConditions = assignPhaseValues(args.seeds, coordinates, directions, plane_info[2::3],
                                          plane_info[0:len(plane_info) - 2:3], plane_info[1:len(plane_info) - 1:3], xyz)
